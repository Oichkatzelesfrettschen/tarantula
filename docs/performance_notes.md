...
To reproduce this benchmark locally, ensure `clang` is installed (for example,
`apt-get install clang` on Debian/Ubuntu) and then compile and run the helper:

```sh
clang -O2 tools/bench_vfs_bufstats.c -o /tmp/bench && /tmp/bench
```
