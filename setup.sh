...
# optional offline mode and selective package sets
OFFLINE_MODE=0
INSTALL_CORE=0
INSTALL_LANGS=0
INSTALL_CROSS=0
...
# default to installing everything when no category flags were supplied
if [ $INSTALL_CORE -eq 0 ] && [ $INSTALL_LANGS -eq 0 ] && [ $INSTALL_CROSS -eq 0 ]; then
  INSTALL_CORE=1
  INSTALL_LANGS=1
  INSTALL_CROSS=1
fi
...
# invoke requested package groups
[ $INSTALL_CORE -eq 1 ] && install_core_tools
[ $INSTALL_LANGS -eq 1 ] && install_language_runtimes
[ $INSTALL_CROSS -eq 1 ] && install_cross_compilers
